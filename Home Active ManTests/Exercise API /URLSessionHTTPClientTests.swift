...
//
//  Created by Josef Santamaria on 2020-12-16.
//
...
class URLSessionHTTPClientTests: XCTestCase {
    ...
    override class func setUp() {
        super.setUp()
        URLProtocolStub.startInterceptRequest()
    }
    override class func tearDown() {
        super.tearDown()
        URLProtocolStub.stopInterceptRequest()
    }
    
    func test_getFromURL_perfomeGETRequestFromURL() {
        let url = anyURL()
    ...
    func test_getFromURL_succedsOnHTTPURLresponseWithData() {
        ...
        makeSUT().get(from: anyURL()) { result in
            switch result {
            case let .success(receivedData, reseivedResponse):
    ...
    private func resultErrorFor(data: Data?, response: URLResponse?, error: Error?, file: StaticString = #filePath, line: UInt = #line) -> Error? {
        ...
        sut.get(from: anyURL()) { result in
            ...
            case let .failure(error):
                receivedError = error
            default:
                XCTFail("Expected failure with \(result) instead", file: file, line: line)
            }
            exp.fulfill()
        }
        wait(for: [exp], timeout: 1.0)
    ...
    private class URLProtocolStub: URLProtocol {
        ...
        static func startInterceptRequest() {
            URLProtocol.registerClass(URLProtocolStub.self)
        }
        
        static func stopInterceptRequest() {
